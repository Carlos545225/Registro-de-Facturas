                ...
                if (typeof readFromGoogleSheets === 'function' && typeof writeToGoogleSheets === 'function' && typeof ensureAuthenticated === 'function') {
                    try {
                        ...
                        json.forEach(row => {
                            ...
                            if(n===1) {
                                // Alimentación - crear/actualizar registro completo
                                const d = parseExcelDate(row.FEC_FACTURA);
                                // Formatear fecha en formato local YYYY-MM-DD sin problemas de zona horaria
                                let dateStr = '';
                                let yearValue = currentYear;
                                let mesValue = currentMonth;
                                
                                if (d && !isNaN(d.getTime())) {
                                    // Extraer año y mes para el filtro
                                    yearValue = d.getFullYear().toString();
                                    mesValue = d.getMonth(); // 0-11
                                    // Formatear fecha usando función auxiliar (sin problemas de zona horaria)
                                    dateStr = formatearFechaLocal(d);
                                }
                                
                                const existing = dataMap.get(id) || {};
                                
                                // Si no se pudo parsear la fecha, usar valores existentes o actuales
                                if (!dateStr) {
                                    dateStr = existing.fecha || '';
                                    if (existing.year) {
...
function parseExcelDate(v) {
    ...
    if (!isNaN(v) && typeof v === 'number') {
        console.log('🔢 Es número serial de Excel:', v);
        // Excel usa serial dates donde 1 = 1 de enero de 1900
        // Pero Excel tiene un bug: considera 1900 como año bisiesto
        // Para corregir esto, usamos el 30 de diciembre de 1899 como epoch
        
        // TRABAJAR COMPLETAMENTE EN HORA LOCAL (NO UTC)
        // Crear fecha base en hora LOCAL (no UTC)
        const excelEpoch = new Date(1899, 11, 30, 12, 0, 0); // 30 de diciembre de 1899 en hora LOCAL (mediodía)
        // Agregar los días (v - 1) en milisegundos
        const date = new Date(excelEpoch.getTime() + (v - 1) * 86400 * 1000);
        
        // Extraer componentes LOCALES directamente (no UTC)
        const year = date.getFullYear();
        const month = date.getMonth();
        const day = date.getDate();
        
        console.log('📆 Extraído de serial:', {year, month, day, fechaOriginal: date});
        
        // Crear nueva fecha en hora local con los componentes extraídos (mediodía para evitar problemas)
        const result = new Date(year, month, day, 12, 0, 0);
        console.log('📅 Resultado serial:', result);
        return result;
    }
